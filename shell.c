...
// Struct to represent a history node
typedef struct HistoryNode {
    char* command;
    struct HistoryNode* next;
} HistoryNode;
...
int execute(char* line) {
    ...
    // You can modify this function to handle the various built-in commands and external commands

    if (strcmp(line, "history") == 0) {
        print_history();
        return 1;  // Continue shell loop
    } else if (strcmp(line, "exit") == 0) {
        return 0;  // Terminate shell loop
    }
...
int execute(char** args) {
    ...
    if (is_builtin(args[0])) {
        int index = 0;
        while (strcmp(args[0], builtin_commands[index]) != 0) {
            if(builtin_commands[index] == "history") {
                print_history();
                return 1;
            }
            index++;
